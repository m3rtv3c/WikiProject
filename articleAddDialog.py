...
from db import get_connection, get_all_article_titles
    ...
    def __init__(self, user_id, parent=None):
        ...
        if not isinstance(user_id, int):
            QMessageBox.critical(
                self,
                "Ошибка",
                f"Передан неверный user_id: {user_id} ({type(user_id).__name__})"
            )
            self.reject()
            return

        self.user_id = user_id  # ID пользователя, который добавляет
        self.images = []
    ...
    def save_article(self):
        ...
        try:
            ...
            cur.execute("""
                ...
                SELECT title, content, NOW(), 'pending', views, id, %s
            ...
            QMessageBox.information(self, "Успешно", "Статья отправлена на модерацию")
            self.accept()
